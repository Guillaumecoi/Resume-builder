...
public interface LayoutSectionItemRepository extends JpaRepository<LayoutSectionItem, Long> {
    ...
    @Query("SELECT r.createdBy FROM LayoutSectionItem lsi JOIN lsi.layoutSubSection lss JOIN layoutSectionRow lsr JOIN lsr.columnSection cs JOIN cs.column c JOIN c.layout l JOIN l.resume r WHERE lsi.id = :id")
